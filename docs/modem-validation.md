...
- Sends modem serial protocol commands (`RESET`, `SEND_FRAME`, `SET_CHANNEL`, `GET_STATUS`, `SCAN_CHANNELS`, `DISPLAY_FRAME_BEGIN`, `DISPLAY_FRAME_CHUNK`).
- Receives and validates modem responses (`MODEM_READY`, `RECV_FRAME`, `EVENT_ERROR`, `STATUS`, etc.).
...
5. Construct a well-formed serial frame with `len` = 1025 (maximum allowed). Use `type` = 0x7F (unknown) and 1024 bytes of padding as body.
6. Send the `len` = 1025 frame to the modem.
...
2. Send a frame with `len` = 1500 (exceeds 1025 max).
...
5. Assert: no display output is triggered by the button press alone.
...
## 10  Display output tests

### T-0900  Reliable display transfer accepted and rendered

**Validates:** MD-0700, MD-0701

**Procedure:**
1. Send `RESET`, wait for `MODEM_READY`.
2. Send `DISPLAY_FRAME_BEGIN` and assert `DISPLAY_FRAME_ACK(next_chunk_index = 0)`.
3. Send eight `DISPLAY_FRAME_CHUNK` messages containing a known 1024-byte test pattern, asserting that each accepted chunk advances `next_chunk_index` by one.
...
### T-0901  Reliable display transfer invalid metadata rejected

**Validates:** MD-0700, MD-0704

**Procedure:**
1. Send `RESET`, wait for `MODEM_READY`.
2. Send a malformed `DISPLAY_FRAME_BEGIN` with an empty body.
3. Assert: `EVENT_ERROR(INVALID_FRAME)` is received.
4. Repeat with a malformed `DISPLAY_FRAME_CHUNK` whose body is not exactly 130 bytes.
5. Assert: `EVENT_ERROR(INVALID_FRAME)` is received and the active display transfer is aborted.
6. Restart the transfer, then send `DISPLAY_FRAME_CHUNK` with `chunk_index = 8`.
7. Assert: `EVENT_ERROR(INVALID_FRAME)` is received and the active display transfer is aborted.

---

### T-0901a  Reliable display transfer duplicate chunk is idempotent
...
1. Send `RESET`, wait for `MODEM_READY`.
2. Send `DISPLAY_FRAME_BEGIN` and assert `DISPLAY_FRAME_ACK(next_chunk_index = 0)`.
3. Send `DISPLAY_FRAME_CHUNK(chunk_index = 0)` and assert `DISPLAY_FRAME_ACK(next_chunk_index = 1)`.
4. Re-send the same chunk 0 again (simulating a lost ACK).
5. Assert: the modem does **not** emit `EVENT_ERROR(INVALID_FRAME)`.
6. Assert: the modem re-sends `DISPLAY_FRAME_ACK(next_chunk_index = 1)` and does not advance transfer state twice.

---

### T-0902  Display updates do not interfere with ESP-NOW

**Validates:** MD-0702

**Procedure:**
1. Establish a baseline ESP-NOW forwarding rate with no display activity.
...
### T-0903  Display updates do not interfere with BLE pairing

**Validates:** MD-0702

**Procedure:**
1. Enable BLE advertising via `BLE_ENABLE`.
2. Initiate a BLE LESC pairing from a phone while repeatedly sending reliable display transfers.
3. Assert: BLE pairing completes successfully.
4. Assert: the display continues updating during the pairing session.

---

### T-0904  No modem-generated display UI

**Validates:** MD-0703, MD-0605

**Procedure:**
1. Reset the modem and record the display state.
2. Exercise button presses, BLE enable/disable, BLE pairing, and ESP-NOW traffic without sending any reliable display transfer.
3. Assert: no new display content is generated by those modem-local events.
4. Send the same completed display transfer under two different modem runtime states.
5. Assert: the rendered output is identical in both cases.

---

### T-0905  OLED write failure emits recoverable error
...
| T-0900 | Reliable display transfer accepted and rendered | MD-0700, MD-0701 |
| T-0901 | Reliable display transfer invalid metadata rejected | MD-0700, MD-0704 |
| T-0901a | Reliable display transfer duplicate chunk is idempotent | MD-0700 |
| T-0902 | Display updates do not interfere with ESP-NOW | MD-0702 |
