...
On program install:
1. Validate map definitions: reject any `map_type` other than 0 or 1 (ND-0606). `map_type` 0 (global variable maps from `.rodata`/`.data`/`.bss` ELF sections) is treated identically to `map_type` 1 (`BPF_MAP_TYPE_ARRAY`) — both are stored as array maps with `max_entries` entries, each `value_size` bytes, keyed by a `u32` index. Global variable maps typically have `max_entries == 1`; `.rodata` and `.data` may carry `initial_data`, while `.bss` is represented by an absent `initial_data` field and therefore starts zero-filled (ND-0607).
