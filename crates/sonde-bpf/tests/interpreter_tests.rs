...
fn test_mov64_imm_exit() {
    ...
    let mut mem = [];
    assert_eq!(
        unsafe { unsafe { execute_program(&prog, &mut mem, &[], &[], false) } }.unwrap(),
        42
    );
}
...
fn test_helper_call() {
    ...
    assert_eq!(
        unsafe { unsafe { execute_program(&prog, &mut mem, helpers, &[], false) } }.unwrap(),
        42
    );
}
...
fn test_unknown_helper() {
    ...
    let mut mem = [];
    assert!(matches!(
        unsafe { execute_program(&prog, &mut mem, &[], &[], false) },
...
fn test_be16() {
    ...
    let mut mem = [];
    let result = unsafe { unsafe { execute_program(&prog, &mut mem, &[], &[], false) } }.unwrap();
...
fn test_read_only_ctx_rejects_store() {
    ...
    let mut mem = [0u8; 16];
    assert!(matches!(
        unsafe { execute_program(&prog, &mut mem, &[], &[], true) },
