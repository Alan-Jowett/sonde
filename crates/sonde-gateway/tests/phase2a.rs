...
async fn t0702_battery_level_tracking() {
    ...
    node.update_telemetry(3300, 1, "0.4.0");
    storage.upsert_node(&node).await.unwrap();
    ...
    node.update_telemetry(2900, 1, "0.4.0");
    storage.upsert_node(&node).await.unwrap();
...
async fn t0702b_battery_history_retention_and_cap() {
    ...
    for i in 0u32..105 {
        node.update_telemetry(3000 + i, 1, "0.4.0");
    }
    storage.upsert_node(&node).await.unwrap();
...
async fn t0703_firmware_abi_version_tracking() {
    ...
    node.update_telemetry(3300, 2, "0.4.0");
    storage.upsert_node(&node).await.unwrap();
    ...
    node.update_telemetry(3300, 3, "0.4.0");
    storage.upsert_node(&node).await.unwrap();
