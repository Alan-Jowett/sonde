...
//!   2.  GW-0600 — AEAD failure state isolation
...
// ═══════════════════════════════════════════════════════════════════════
//  Gap 2 — GW-0600: AEAD failure state isolation
// ═══════════════════════════════════════════════════════════════════════

/// After AEAD verification failure from a known `key_hint`, the gateway's
...
async fn t0600_hmac_failure_state_unchanged() {
    ...
    // Send a frame with the correct key_hint but corrupted GCM tag.
    ...
    assert_eq!(
        ...
        "next_expected_seq must not change after AEAD failure"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        "firmware_abi_version must not change after AEAD failure"
    );
