...
- Append a new history row in `ActualState`. Use `PartitionKey = "g:" +
  entity_id` (hex-encoded gateway_id) and a reverse-timestamp `RowKey`
  generated by `next_history_row_key()` — the same keying scheme as
  node rows. `load_gateway_actual_state` returns only the latest row.
...
4. Send updated ACTUAL_STATE — verify a second history row is appended
   and `load_gateway_actual_state` returns the latest row.

**Pass criteria:** Gateway state stored as append-only history; latest row reflects most recent update.
...
1. Read gateway ACTUAL_STATE from Azure `actualstate` Table
   (query `PartitionKey` starting with `"g:"`, select latest row per
   gateway partition via `latestByPartition`).
