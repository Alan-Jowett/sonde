...
3. Timestamps displayed in human-readable output MUST be formatted as human-readable UTC dates (e.g., `YYYY-MM-DD HH:MM:SS UTC`), not raw milliseconds. JSON output retains the numeric `_ms` fields for machine consumption.
...
### GW-1101a  Modem-ready display banner

**Priority:** Must
**Source:** modem-protocol.md §4.7a, §6.2; GW-1303

**Description:**
After the modem startup handshake completes successfully, the gateway MUST render a version banner into a full 128×64 1-bit framebuffer and send it to the modem via the reliable display-transfer protocol (`DISPLAY_FRAME_BEGIN` + eight acknowledged `DISPLAY_FRAME_CHUNK` messages). The banner text MUST be generated by the gateway, not by the modem firmware, and MUST include the gateway crate semantic version in the form `Sonde Gateway v<semver>`.

Whenever the gateway re-initializes the modem after a serial reconnect or warm reboot (GW-1103), it MUST send the same version banner again after the startup handshake completes.

**Acceptance criteria:**

1. After a successful initial modem startup handshake, the gateway sends exactly one reliable display transfer carrying the rendered version banner.
2. The displayed banner text includes the gateway crate semantic version string, formatted as `Sonde Gateway v<semver>`.
3. The gateway performs the text-to-framebuffer rendering; the modem receives only the opaque framebuffer bytes.
...
On receiving a recoverable `EVENT_ERROR` message from the modem display path, the gateway MUST log the display fault and continue operating. A recoverable display fault MUST NOT trigger a modem reset or interfere with radio, BLE, or serial processing.

If a reliable display transfer to the modem exhausts its ACK retransmission budget, the gateway MUST treat that condition as a modem transport failure, not as a recoverable display-path warning. The gateway MUST abandon the transfer and enter the same reconnect / re-initialization path used for serial transport failure.
...
10. `EVENT_ERROR` display faults are logged as recoverable warnings and do not trigger a reset or reconnect.
11. Exhausting the ACK retry budget for a reliable display transfer triggers the same modem recovery path as a transport failure.
...
6. When `APP_DATA` does not match any handler: if active connector subscribers exist (e.g. a companion process), the data is forwarded to them and a DEBUG log is emitted with `node_id`, `program_hash`, `handler_count`, and `connector_subscribers`. If no connector subscribers exist either, a WARN log is emitted with `node_id`, `program_hash`, `handler_count`, and the message indicates the data was dropped.
...
**Description:**  
The `.deb` package SHOULD include a systemd unit file and `postinst` / `prerm` scripts that create a `sonde` system user, enable and start the service, and configure the environment file at `/etc/sonde/environment` with a default `SERIAL_PORT=/dev/ttyUSB0`. The operator SHOULD edit `/etc/sonde/environment` to set `SERIAL_PORT` to the correct device if it differs from the default. The `prerm` script stops and disables the service on package removal (but not on upgrade).
...
2. Release builds (tag `v*`) are tagged `latest` and the semver version (e.g., `0.8.0`).
