...
const DASHBOARD_RUNTIME = typeof module !== 'undefined' && module.exports
  ? require('./dashboard-runtime.js')
  : globalThis.SondeDashboardRuntime;

if (!DASHBOARD_RUNTIME) {
  throw new Error('SondeDashboardRuntime is not available.');
}
...
const createDefaultDashboardsArray = DASHBOARD_RUNTIME.createDefaultDashboardsArray;
const normalizeDashboardTimeRange = DASHBOARD_RUNTIME.normalizeDashboardTimeRange;
const getDashboardMetricCount = DASHBOARD_RUNTIME.getDashboardMetricCount;
const getDashboardMetrics = DASHBOARD_RUNTIME.getDashboardMetrics;
const renderDashboardTabs = DASHBOARD_RUNTIME.renderDashboardTabs;
const renderDashboardContent = DASHBOARD_RUNTIME.renderDashboardContent;
const renderChartCard = DASHBOARD_RUNTIME.renderChartCard;
const getDashboardTimeRangeBounds = DASHBOARD_RUNTIME.getDashboardTimeRangeBounds;

function normalizeDashboard(dashboard, index) {
  return DASHBOARD_RUNTIME.normalizeDashboard(dashboard, index);
}

function serializeDashboard(dashboard, index) {
  return DASHBOARD_RUNTIME.serializeDashboard(dashboard, index);
}
...
function validateVariableName(name, existingNames) {
  ...
  if (!validation.valid && validation.error === 'Variable name must start with a letter and contain only letters, numbers, and underscores') {
...
function validateExpression(expression, availableVariables) {
  return DASHBOARD_RUNTIME.validateExpression(expression, availableVariables);
}

function isVariableUsedInExpression(variableName, expression) {
  return DASHBOARD_RUNTIME.isVariableUsedInExpression(variableName, expression);
}

function normalizeEnvironmentRecord(env) {
  return DASHBOARD_RUNTIME.normalizeEnvironmentRecord(env, {
    sanitizeSensorDataPreferences,
    validateExpressionFn: validateExpression,
  });
}

function serializeEnvironmentRecord(env) {
  return DASHBOARD_RUNTIME.serializeEnvironmentRecord(env, {
    sanitizeSensorDataPreferences,
    validateExpressionFn: validateExpression,
  });
}
...
function buildEnvironmentExportData(env) {
  return DASHBOARD_RUNTIME.buildEnvironmentExportData(env, {
    sanitizeSensorDataPreferences,
    validateExpressionFn: validateExpression,
  });
}
...
function validateImportedDashboard(dashboard, index) {
  return DASHBOARD_RUNTIME.validateImportedDashboard(dashboard, index, {
    validateVariableNameFn: validateVariableName,
  });
}
...
function canAddMetric(dashboard) {
  if (getDashboardMetricCount(dashboard) >= 10) {
    return {
      allowed: false,
...
async function renderMetricCharts(dashboard, deps = {}) {
  return DASHBOARD_RUNTIME.renderMetricCharts(dashboard, {
    ...deps,
    destroyChartFn: deps.destroyChartFn || destroyDashboardChart,
    evaluateMetricTimeSeriesFn: deps.evaluateMetricTimeSeriesFn || evaluateMetricTimeSeries,
    storeChartInstanceFn: deps.storeChartInstanceFn || ((chartIndex, chart) => {
      APP_DASHBOARD_STATE.metricCharts[chartIndex] = chart;
    }),
  });
}
...
async function evaluateMetricTimeSeries(metric, variables, timeRange, deps = {}) {
  ...
  const fetchVariableDataFn = async (usedVariables, requestedTimeRange, runtimeDeps) => {
    const fetchResult = await (deps.fetchVariableDataFn || fetchVariableData)(usedVariables, requestedTimeRange || { preset: '24h' }, runtimeDeps);
    if (fetchResult.errors && fetchResult.errors.length > 0) {
      throw new Error(fetchResult.errors[0]);
    }
    return fetchResult.data;
  };
  const result = await DASHBOARD_RUNTIME.evaluateMetricTimeSeries(metric, variables, timeRange, {
    ...deps,
    parserFactory,
    fetchVariableDataFn,
  });
  if (typeof result.error === 'string' && result.error.startsWith('Failed to fetch dashboard data: ')) {
    return {
      ...result,
      error: result.error.slice('Failed to fetch dashboard data: '.length),
    };
  }
  return result;
}
...
function attachDashboardHandlers() {
  ...
  document.querySelectorAll('[data-add-metric]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (!check.allowed && !window.confirm(check.warning)) return;
      showAddMetricModal(parseInt(btn.dataset.addMetric, 10));
    });
  });
  ...
  document.querySelectorAll('[data-edit-metric-chart]').forEach(btn => {
    btn.addEventListener('click', () => {
      showEditMetricModal(
        parseInt(btn.dataset.editMetricChart, 10),
        parseInt(btn.dataset.editMetricIndex, 10),
      );
    });
  });

  document.querySelectorAll('[data-delete-metric-chart]').forEach(btn => {
    btn.addEventListener('click', () => {
      deleteMetric(
        parseInt(btn.dataset.deleteMetricChart, 10),
        parseInt(btn.dataset.deleteMetricIndex, 10),
      );
    });
  });
}
...
function showAddChartModal() {
  const env = loadActiveEnvironment();
  const dashboard = env.dashboards[APP_DASHBOARD_STATE.activeDashboardIndex];
  const name = window.prompt('Enter chart name:', `Chart ${dashboard.charts.length + 1}`);
  if (!name || !name.trim()) {
    return;
  }
  dashboard.charts.push(createDefaultChart(name.trim()));
  const environments = loadEnvironments();
  persistDashboardEnvironment(env, environments);
  renderActiveTab();
}

function editChart(chartIndex) {
  const env = loadActiveEnvironment();
  const dashboard = env.dashboards[APP_DASHBOARD_STATE.activeDashboardIndex];
  const chart = dashboard.charts[chartIndex];
  if (!chart) {
  ...
  const name = window.prompt('Enter new chart name:', chart.name);
  if (!name || !name.trim()) {
    return;
  }
...
function deleteChart(chartIndex) {
  const env = loadActiveEnvironment();
  const dashboard = env.dashboards[APP_DASHBOARD_STATE.activeDashboardIndex];
  const chart = dashboard.charts[chartIndex];
  if (!chart) {
  ...
  if (!window.confirm(`Delete chart "${chart.name}" and its metrics?`)) {
    return;
  }
...
function deleteVariable(index) {
  ...
  const usedInMetrics = getDashboardMetrics(dashboard).filter(m =>
...
function showAddMetricModal(chartIndex) {
  const env = loadActiveEnvironment();
  const dashboard = env.dashboards[APP_DASHBOARD_STATE.activeDashboardIndex];
  const chart = dashboard.charts[chartIndex];

  if (!chart) {
    alert('Add a chart before creating metrics.');
    return;
  }
...
function showEditMetricModal(chartIndex, metricIndex) {
  const env = loadActiveEnvironment();
  const dashboard = env.dashboards[APP_DASHBOARD_STATE.activeDashboardIndex];
  const chart = dashboard.charts[chartIndex];
  const metric = chart?.metrics[metricIndex];
  if (!chart || !metric) {
    return;
  }
  ...
  if (newDisplayName) nextMetric.displayName = newDisplayName;
  
  const availableVars = dashboard.variables.map(v => v.name).join(', ');
  const newExpression = window.prompt(`Enter expression (variables: ${availableVars}):`, nextMetric.expression);
  if (newExpression) {
    ...
    nextMetric.expression = newExpression;
  }
  
  const newColor = window.prompt('Enter color (hex):', nextMetric.color);
  if (newColor) nextMetric.color = newColor;

  let nextChartIndex = chartIndex;
  if (dashboard.charts.length > 1) {
    const chartChoices = dashboard.charts.map((entry, index) => `${index + 1}: ${entry.name}`).join('\n');
    const chartSelection = window.prompt(`Enter chart number for this metric:\n${chartChoices}`, String(chartIndex + 1));
    if (chartSelection) {
      nextChartIndex = Number.parseInt(chartSelection, 10) - 1;
      if (!Number.isInteger(nextChartIndex) || nextChartIndex < 0 || nextChartIndex >= dashboard.charts.length) {
        alert('Chart number must match an existing chart.');
        return;
      }
    }
  }

  if (nextChartIndex === chartIndex) {
    chart.metrics[metricIndex] = nextMetric;
  } else {
    chart.metrics.splice(metricIndex, 1);
    dashboard.charts[nextChartIndex].metrics.push(nextMetric);
  }
...
function deleteMetric(chartIndex, metricIndex) {
  const env = loadActiveEnvironment();
  const dashboard = env.dashboards[APP_DASHBOARD_STATE.activeDashboardIndex];
  const chart = dashboard.charts[chartIndex];
  const metric = chart?.metrics[metricIndex];
