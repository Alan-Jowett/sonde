...
| File | Description |
|------|-------------|
| `ir/` | Intermediate Representation YAML files (IR-0 through IR-3) |
| `bom-jlcpcb.csv` | Bill of Materials in JLCPCB format |
| `cpl-jlcpcb.csv` | Component Placement List (header only — coordinates populated after layout) |
| `README.md` | This file |

KiCad project files (`.kicad_pro`, `.kicad_sch`, `.kicad_pcb`) will be
generated by the `sonde-kicad` tool from the IR files.

## How to Order PCBs

### Step 1: Generate KiCad Files

1. Run `sonde-kicad build --ir-dir hw/carrier-board/ir/` to generate
   the schematic and PCB layout from IR files.
2. Open the generated `.kicad_sch` and `.kicad_pcb` in KiCad 8 to review.
