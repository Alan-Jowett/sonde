...
4. For `WAKE` messages: accept (WAKE replay is tolerated — see [security.md §4.6](security.md)). For post-WAKE messages: extract sequence number from header; if `seq ≤ last_accepted_seq` → silently discard.
...
The gateway maintains a **per-node monotonic counter** (`last_accepted_seq`) for replay protection. See [security.md §4](security.md) for the full design rationale.

| Parameter | Value |
|---|---|
| Nonce (WAKE only) | 64-bit, randomly generated by hardware RNG; binds gateway response to this wake event |
| Sequence numbers (post-WAKE) | 64-bit monotonic counter; gateway assigns starting value in COMMAND response |
| Gateway persistence | `last_accepted_seq` MUST be persisted durably per node |
| Node persistence | None required — gateway assigns starting sequence each wake cycle |

Nodes do not need persistent replay-protection storage:
- WAKE messages use a fresh random nonce per wake cycle.
- The gateway assigns a starting sequence number (always greater than `last_accepted_seq`) in its COMMAND response.
- The node increments the sequence for each subsequent outbound message (GET_CHUNK, PROGRAM_ACK, APP_DATA).
- The gateway rejects any message with `seq ≤ last_accepted_seq`.
