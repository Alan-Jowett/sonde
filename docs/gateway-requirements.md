...
**Description:**
After the modem startup handshake completes successfully, the gateway MUST render a version banner into a full 128×64 1-bit framebuffer and send it to the modem via `DISPLAY_FRAME`. The banner text MUST be generated by the gateway, not by the modem firmware, and MUST include the gateway crate semantic version in the form `Sonde Gateway v<semver>`.
...
1. After a successful initial modem startup handshake, the gateway sends exactly one `DISPLAY_FRAME` carrying the rendered version banner.
...
4. After a serial reconnect recovery or warm reboot recovery, the gateway sends the version banner again without requiring operator action.
