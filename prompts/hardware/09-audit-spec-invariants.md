...
1. The device MUST always be remotely recoverable — for every reachable state and every failure mode, there exists a recovery path to a state that accepts remote commands. 2. No single point of failure in the update mechanism — power loss at any point during firmware update must not brick the device. 3. Deep sleep current MUST NOT exceed 20µA under any configuration.

**Context**: <!-- ADD CONTEXT — what the system does, hardware constraints, operational environment -->

**Audience**: <!-- SPECIFY AUDIENCE — e.g., "spec authors", "firmware engineers", "safety reviewers" -->
