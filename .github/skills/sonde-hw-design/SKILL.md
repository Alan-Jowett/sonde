...
description: >
  End-to-end hardware design workflow for Sonde sensor/carrier boards.
  Guides through requirements, component selection, schematic, PCB
  layout, and manufacturing with adversarial audits at every gate.
  Multi-phase interactive workflow.
---

<!-- Generated by PromptKit — edit with care -->

<!-- Dependencies (loaded automatically via instructions):
  Instructions: sonde-guardrails (applyTo: "**")
  Step files loaded just-in-time per phase (see Phase Loading table)
-->
...
**Project**: <name of this Sonde carrier/sensor board>

**Description**:
<what sensors and features this board needs — e.g. soil moisture + temperature + battery holder for outdoor deployment>
...
- The `sonde-guardrails` instructions (anti-hallucination +
  self-verification) are loaded automatically and apply to every phase
