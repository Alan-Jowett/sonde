...
| **Nonce** | A 64-bit random value generated by a node's hardware RNG for each `WAKE` message. Identifies the session. |
...
2. The `NOP` command carries `starting_seq` and `command_type` in the CBOR payload; no command-specific payload is included.
...
2. The command carries `starting_seq` and `command_type` in the CBOR payload; no command-specific payload is included.
...
1. The gateway responds to `GET_CHUNK { sequence_number, chunk_index }` with `CHUNK { sequence_number, chunk_index, chunk_data }`.
...
**Description:**  
When forwarding `APP_DATA` to a handler, the gateway MUST send a `DATA` message containing: `msg_type` (0x01), `request_id` (correlation ID), `node_id` (opaque admin-assigned identifier), `program_hash`, `data` (the opaque blob), and `timestamp`.
