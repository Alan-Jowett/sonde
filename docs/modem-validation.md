...
- Sends modem serial protocol commands (`RESET`, `SEND_FRAME`, `SET_CHANNEL`, `GET_STATUS`, `SCAN_CHANNELS`, `DISPLAY_FRAME`).
...
### T-0900  DISPLAY_FRAME accepted and rendered

**Validates:** MD-0700, MD-0701

**Procedure:**
1. Send `RESET`, wait for `MODEM_READY`.
2. Send a `DISPLAY_FRAME` containing a known 1024-byte test pattern.
3. Observe the OLED output (or capture the display driver's mock I²C transactions).
4. Assert: the rendered pixels match the source framebuffer orientation and bit packing.
5. Assert: no `EVENT_ERROR` is emitted.

---

### T-0901  DISPLAY_FRAME invalid length rejected
...
2. Repeat the same traffic load while continuously sending `DISPLAY_FRAME` updates.
...
**Procedure:**
1. Enable BLE advertising via `BLE_ENABLE`.
2. Initiate a BLE LESC pairing from a phone while repeatedly sending `DISPLAY_FRAME` updates.
3. Assert: BLE pairing completes successfully.
...
2. Exercise button presses, BLE enable/disable, BLE pairing, and ESP-NOW traffic without sending any `DISPLAY_FRAME`.
3. Assert: no new display content is generated by those modem-local events.
4. Send the same `DISPLAY_FRAME` under two different modem runtime states.
...
**Procedure:**
1. Configure the display transport to inject an I²C write failure during a `DISPLAY_FRAME` update.
2. Send a valid `DISPLAY_FRAME`.
...
| T-0900 | DISPLAY_FRAME accepted and rendered | MD-0700, MD-0701 |
| T-0901 | DISPLAY_FRAME invalid length rejected | MD-0700, MD-0704 |
