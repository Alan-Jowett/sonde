...
| `0x06` | `DIAG_REQUEST` | Pairing-time diagnostic request. Authenticated with phone PSK. See §5.8. |
...
| `0x85` | `DIAG_REPLY` | Pairing-time diagnostic reply. Authenticated with phone PSK. See §5.9. |
...
`DIAG_REQUEST` and `DIAG_REPLY` use a **separate CBOR key namespace** scoped to their respective `msg_type` values, following the same pattern as `PEER_REQUEST` / `PEER_ACK`.
...
|---|---|---|
| `0x01` | `DIAG_RSSI` | Node→gateway RF link quality measurement. |
...
| `firmware_version` | tstr | Yes | Firmware version string (e.g., `"0.7.0"`). ASCII-only, maximum 32 bytes. Derived from `CARGO_PKG_VERSION` at compile time. |
...
| Field | CBOR type | Required | Description |
|---|---|---|---|
| `diagnostic_type` | uint | Yes | Diagnostic operation requested. Currently only `0x01` (`DIAG_RSSI`). |

The `key_hint` in the frame header identifies a phone PSK — the gateway performs phone PSK lookup (same path as `PEER_REQUEST`). The `nonce` field carries a random value generated by the pairing tool.
