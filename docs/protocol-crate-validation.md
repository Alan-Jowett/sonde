...
**Procedure:**
1. Create `NodeMessage::Wake { firmware_abi_version: 1, program_hash: vec![0xAA; 32], battery_mv: 3300, firmware_version: "0.7.0".to_string() }`.
2. Encode to CBOR.
...
**Procedure:**
1. Encode a `NodeMessage::Wake` with `firmware_abi_version=1`, `program_hash=[0x42; 32]`, `battery_mv=3300`, `firmware_version="0.7.0"`, and `blob=Some([0xAA, 0xBB])`.
