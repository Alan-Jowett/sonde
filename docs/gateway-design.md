...
This section covers platform packaging (MSI and `.deb`), system PATH registration, COM port auto-detection, and service registration.
...
> **Requirements:** GW-1700 (DIAG_REQUEST reception), GW-1701 (DIAG_REQUEST session bypass), GW-1702 (RSSI measurement), GW-1704 (DIAG_REPLY construction), GW-1706 (diagnostic logging).
...
1. Build CBOR payload: `{ 1: diagnostic_type, 2: rssi_dbm }` using deterministic encoding (RFC 8949 §4.2).
2. Frame header:
...
- `DIAG_REPLY` sent: RSSI value, target MAC.
...
| Trigger | Tags |
|---------|------|
| Release tag (`v*`) | `latest`, semver (e.g., `0.7.0`), `sha-<short>` |
