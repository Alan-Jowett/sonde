...
7. **Prepare the active GPIO state and capture current-cycle battery value**: Using the provisioned board layout (§10.3), drive `sensor_enable` low, drive the provisioned I2C and 1-Wire data pins high, configure `battery_adc` as an input, then wait for the rail to settle. The canonical carrier board's `VBAT_SENSE` network includes a 100 nF filter capacitor behind a 100 kΩ / 100 kΩ divider, so firmware waits 50 ms when `battery_adc` is provisioned there; layouts without battery sensing still use a 1 ms bus-stabilization delay. The firmware then captures the current-cycle battery value from the provisioned `battery_adc` pin (or uses the fallback `3300` mV when no ADC pin is assigned) and stores the value in RTC-retained state for the next wake. This battery-capture step happens on every successful non-reboot wake cycle, even if no resident BPF program is installed.
8. **Dispatch command**:
