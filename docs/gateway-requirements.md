...
3. If the removed handler has a running process, the process is terminated (SIGTERM, then SIGKILL after timeout) before the handler is removed from the routing table.
