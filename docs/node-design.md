...
Uses `sonde_protocol::aead_seal()` with a constructed `FrameHeader`, the node's PSK, and the node's AEAD implementation:
...
let frame = sonde_protocol::aead_seal(
    &header, &payload_cbor, psk, &aead_impl,
);
```
...
| **Frame codec** | `aead_seal()`, `aead_open()`, header parsing at fixed offsets |
