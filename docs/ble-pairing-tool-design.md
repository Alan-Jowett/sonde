...
- Location: `%APPDATA%\sonde\pairing-aead.json`
...
- Writes are atomic (write-to-temp, then rename) to prevent corruption on crash
...
- **GATT write failures (WinRT):** Write failures are surfaced immediately to the caller — no client-side retry (PT-1003).  The modem handles the pre-authentication race server-side: it calls `ble_gap_security_initiate()` on connect to start LESC pairing, and buffers one pre-auth GATT write until the operator confirms the passkey (MD-0409).  If a stale WinRT Bluetooth cache causes auth-related write rejections (e.g., `HRESULT 0x80650005` from a previous firmware that used encrypted characteristics), the operator should remove the stale Bluetooth pairing in Windows Settings and retry.
