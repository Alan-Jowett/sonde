...
(function (root, factory) {
  ...
  function normalizeEnvironmentRecord(env, deps = {}) {
    ...
    const normalized = {
      ...
      loginEndpoint: typeof env?.loginEndpoint === 'string' ? env.loginEndpoint : '',
      kioskSetupClientId: typeof env?.kioskSetupClientId === 'string' ? env.kioskSetupClientId : '',
  ...
  function serializeEnvironmentRecord(env, deps = {}) {
    ...
    return {
      ...
      ...(normalized.loginEndpoint ? { loginEndpoint: normalized.loginEndpoint } : {}),
      ...(normalized.kioskSetupClientId ? { kioskSetupClientId: normalized.kioskSetupClientId } : {}),
  ...
  function buildEnvironmentExportData(env, deps = {}) {
    ...
    return {
      ...
      ...(normalizedEnv.loginEndpoint ? { loginEndpoint: normalizedEnv.loginEndpoint } : {}),
      ...(normalizedEnv.kioskSetupClientId ? { kioskSetupClientId: normalizedEnv.kioskSetupClientId } : {}),
  ...
  async function renderMetricCharts(dashboard, deps = {}) {
    ...
    for (let chartIndex = 0; chartIndex < normalizedDashboard.charts.length; chartIndex++) {
      ...
      for (const metric of chart.metrics) {
        ...
        const plottedPoints = downsamplePointsFn(result.points, 500)
