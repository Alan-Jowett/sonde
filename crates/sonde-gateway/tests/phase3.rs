...
async fn store_test_program(storage: &InMemoryStorage, bytecode: &[u8]) -> Vec<u8> {
    ...
    let record = lib.ingest_unverified(cbor, VerificationProfile::Resident).unwrap();
...
async fn t0402_prevail_verification_resident_pass() {
    let elf_bytes = include_bytes!("../../../test-programs/nop.o");
    let lib = ProgramLibrary::new();
...
async fn t0608_frame_overhead_budget() {
    ...
    let response = gateway.process_frame(&wake_frame, node.peer_address()).await;
...
async fn t0701_stale_program_detection() {
    ...
    let hash_a = store_test_program(&*storage, &[0x95, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]).await;
    let hash_b = store_test_program(&*storage, &[0xB7, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]).await;
    ...
    let resp = gateway.process_frame(&wake, node.peer_address()).await.unwrap();
...
async fn t1002_export_import_round_trip() {
    ...
    let hash = store_test_program(&*storage_a, &[0x95, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]).await;
...
async fn t1003_concurrent_node_handling() {
    ...
    for handle in handles {
        ...
        assert!(
            resp.is_some(),
            "node {i} must receive a response"
        );
        let raw = resp.unwrap();
        let decoded = decode_frame(&raw).unwrap();
        assert!(
